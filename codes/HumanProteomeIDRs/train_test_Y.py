"""
train_test_Y.py  –  Full 20-amino-acid model for Tyrosine (Y) on Human IDR data

Dataset   : 9-residue windows centred on Y residues, extracted from human IDRs.
            Pre-split into 80 % training and 20 % test sets.

Data files:
    data/Y_rows_train.csv  (space-separated, no header)
    data/Y_rows_test.csv   (same format)
    col 0 – 9-residue sequence (middle pos 5 = Y)
...
Note on scaling: Y has a lower intrinsic insertion propensity than F or W, shifting
the natural product into a lower range.  A smaller SCALE_FACTOR = (10^1)^(1/8) ≈ 1.33
is used (vs. (10^6)^(1/8) for F and W) to keep the optimisation numerically stable.

Amino-acid groups:
    group1  VLIMFWY  hydrophobic / aromatic
    group2  GPCA     helix-breaker / small apolar
    group3  STNQH    polar uncharged
    group4  KR       positively charged
    group5  DE       negatively charged

Outputs   : results/Y_params.csv     –  trained q values for all 20 AAs
...
TRAIN_FILE  = os.path.join(os.path.dirname(__file__), "data", "Y_rows_train.csv")
TEST_FILE   = os.path.join(os.path.dirname(__file__), "data", "Y_rows_test.csv")
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results")
OUT_CSV     = os.path.join(RESULTS_DIR, "Y_params.csv")
...
# Y-specific scale factor: smaller than F/W because Y's product naturally
# falls in a lower numerical range (lower insertion propensity).
# SCALE = (10^1)^(1/8) ≈ 1.33
...
def model_prediction(seq, params, Q_th=1.0, n=1.0):
    ...
    Multiplies q values over 8 context positions; clamps each to 1e-12
    to prevent underflow; passes the product through the sigmoid.
    """
